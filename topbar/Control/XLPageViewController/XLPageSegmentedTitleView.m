...
- (void)initSegmentedWithConfig:(XLPageViewControllerConfig *)config {
    ...
    self.segmentedControl.tintColor = config.segmentedTintColor;
     ...
     //背景
     self.segmentedControl.backgroundColor = config.segmentedBgColor;

     //正常字体大小，颜色
     [self.segmentedControl  setTitleTextAttributes:@{NSForegroundColorAttributeName:config.titleNOcolor,NSFontAttributeName:config.titleNOdFont} forState:UIControlStateNormal];
    
     //选择后字体大小，颜色
     [self.segmentedControl setTitleTextAttributes:@{NSForegroundColorAttributeName:config.titleSedcolor,NSFontAttributeName:config.titleSedFont}forState:UIControlStateSelected];
    
//     //边框宽度，重新画边框，若不重新画，可能会出现圆角处无边框的情况
//     self.segmentedControl.layer.borderWidth = 0.5;
//
//     //边框颜色
//      self.segmentedControl.layer.borderColor = [UIColor colorWithRed:5.0/255 green:86.1/255 blue:198.0/255 alpha:1].CGColor;
    
     //选择器背景颜色
//     if (@available(iOS 13.0, *)) {
//
//        self.segmentedControl.selectedSegmentTintColor = [UIColor colorWithRed:5.0/255 green:86.1/255 blue:198.0/255 alpha:1];
//
//     } else {
//
