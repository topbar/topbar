...
@interface ZLAdvertViewController ()<UIWebViewDelegate,WKNavigationDelegate,WKUIDelegate,UINavigationControllerDelegate>
...
- (void)createWebView{
    ...
    NSString *dicPath = [documentsPath stringByAppendingPathComponent:@"AD.plist"];
    NSDictionary *dic = [NSDictionary dictionaryWithContentsOfFile:dicPath];
    NSString *linkUrl = [dic objectForKey:@"id"];//开机启动画面超连ID或地址
    
    NSString *api_url = [NSString stringWithFormat:@"Placard_list_h5.php?id=%@",linkUrl];
    NSString *urlString =[APIURL stringByAppendingString:api_url];
    //NSLog(@"%@",urlString);
    
   // NSString *urlString = [NSString stringWithFormat:@"http://www.topbar.cn"];
