...
@interface LimitA__ViewController ()

@end

@implementation LimitA__ViewController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath//这个是必需的，
{
     ...
     if (indexPath.section == 0)
    {
        ...
        if (indexPath.row == 0)
        {
            cell.textLabel.text = [NSString stringWithFormat:@"输入金额"];
            LimitA = [[UITextField alloc]initWithFrame:CGRectMake(130,10,200, 40)];
            LimitA.textColor = [UIColor colorWithRed:5.0/255 green:33.1/255 blue:89.0/255 alpha:1];
            LimitA.delegate = self;//限制输入字符,配合方法才能使用
            LimitA.keyboardType = UIKeyboardTypeDecimalPad;//弹出全数字键盘
            LimitA.text =_LimitA_type;

            if (_LimitA_type == NULL || [_LimitA_type isEqualToString:@""]) {
                            //占位符颜色
                ...
                NSMutableAttributedString *attPlaceA = [[NSMutableAttributedString alloc] initWithString:@"请输入你的金额" attributes:attDicA];
                LimitA.attributedPlaceholder = attPlaceA;
                [cell.contentView addSubview: LimitA];
            }
            else
            {
                            //占位符颜色
                NSMutableDictionary *attDicA = [@{NSForegroundColorAttributeName:[UIColor colorWithRed:5.0/255 green:33.1/255 blue:89.0/255 alpha:0.2], NSFontAttributeName:[UIFont systemFontOfSize:16]} mutableCopy];
                NSMutableAttributedString *attPlaceA = [[NSMutableAttributedString alloc] initWithString:_LimitA_type attributes:attDicA];
                LimitA.attributedPlaceholder = attPlaceA;
                [cell.contentView addSubview: LimitA];
            }
...
-(void)Edit_password
{
    ...
    [dic_notice setObject:LimitA.text forKey:@"LimitA"];
    //准备要添加存入的字典
    ...
    NSString * Urladd =[NSString stringWithFormat:@"action=edit_LimitA&username=%@&LimitA=%@",username,LimitA.text];
    //NSLog(@"URL==%@",Urladd);
...
- (void)left
{
    //[self.navigationController popViewControllerAnimated:YES];
        for (UIViewController *controller in self.navigationController.viewControllers) {
            if ([controller isKindOfClass:[Account_ViewController class]]) {
                Account_ViewController *vc =(Account_ViewController *)controller;
                [self.navigationController popToViewController:vc animated:YES];
    //            直接调用MyViewController里面的加载数据的方法即isRefreshData
                vc.LimitA = LimitA.text ;
                [vc LimitA_S];//POP回去刷新页面
    //            若是需要传值的直接属性
               // vc.name = @"你好！帅哥";
            }
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    //获取New_Password输入的长度，
    NSString * user_lengt = [LimitA.text stringByReplacingCharactersInRange:range withString:string];
    LimitA_lengt = [[NSString stringWithFormat:@"%lu",(unsigned long)user_lengt.length] intValue];
