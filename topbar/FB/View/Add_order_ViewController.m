...
@interface Add_order_ViewController ()

@end

@implementation Add_order_ViewController
...
-(void)Header{
    ...
    //Title.text = @"请付款";
    //创建富文本
    NSMutableAttributedString *attri = [[NSMutableAttributedString alloc] initWithString: @" 请付款"];
    ...
    //UILable更改部分字体颜色
    NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"请在%@s内付款给卖家",time]];
    ...
    //修改颜色
    [string addAttribute:NSForegroundColorAttributeName value:[UIColor orangeColor] range:NSMakeRange(2, 6)];
...
//        [HeadPic addSubview:cc];
    ...
    //---------
         ...
         NSString *numA = _pri;
         //NSLog(@"--第二种转换后的格式是--- %@",[moneyFormatter stringFromNumber:[NSNumber numberWithDouble:num.doubleValue]]);
         NSString * prisA  = [moneyFormatterA stringFromNumber:[NSNumber numberWithDouble:numA.doubleValue]];
         //Title.text = [NSString stringWithFormat:@"¥%@",pris];
...
//    [self.view addSubview:rbtn];
    ...
    [QbuttonA setTitle:@"我已付款成功" forState:UIControlStateNormal];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath//这个是必需的，
{
     ...
     if (indexPath.section == 0)
    {
        ...
        if (indexPath.row == 2)
        {
            ...
            _order_num =  [NSString stringWithFormat:@"TBG%@%d",str,Vol_X];
        ...
        if (indexPath.row == 3)
        {
            cell.textLabel.text = [NSString stringWithFormat:@"在转帐过程中请勿备注BTC、USDT、火币等信息,防止汇款被拦截、银行卡被冻结等问题"];
...
//
       ...
       // NSLog(@"btn_type===%@",_Btn_type);
        ...
        __block Add_order_ViewController *blockSelf = self;//创建一个本地变量blockSelf，指向self，然后用结构体语法访问实例变量
...
//
        ...
        APopView  = [[Pop_rz alloc]init];
        APopView.type = @"6"; //传值，到底是认证时间还是认证等级，因为调用同一个窗口。 0 认证等级（默认）  、1 认证时间
...
//
        ...
        APopView  = [[Pop_rz alloc]init];
        APopView.type = @"7"; //传值，到底是认证时间还是认证等级，因为调用同一个窗口。 0 认证等级（默认）  、1 认证时间
        //APopView.currentTime = [NSString stringWithFormat:@"%ld",(long)currentTime];
...
-(void)Add_order
{
        ...
        NSString * Urladd =[NSString stringWithFormat:@"action=order&username=%@&pri=%@&vol=%@&total=%@&btc_type=%@&pay_type=%@&pay_name=%@&pay_num=%@&m_anther=%@&m_name=%@&order_num=%@&type=%@&buy_sale_type=0",username_s,_pri,_vol,_tatol,_Currency,_type,_car_name,_car_num,_Another,_username,_order_num,_text_num];  //tpe=0 取消订单， type=1 我已付款（下单成功） buy_sale_type=0购买 buy_sale_type=1出售
