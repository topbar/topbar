...
@interface Hy_order_s_ViewController ()

@end

@implementation Hy_order_s_ViewController
...
-(void)Nav_UI
{
    //-------自己定义Nav
      ...
      NSString * titles =[NSString stringWithFormat:@"出售%@",_Currency];
      if (_Currency == nil) {
         [title setText:@"出售USDT"];
      } else {
...
//        [HeadPic addSubview:cc];
    ...
    //---------
         _Vol_A = [[UITextField alloc]initWithFrame:CGRectMake(10, 15, ScreenWidth-40-20, 50)];
          ...
          NSMutableAttributedString *attPlaceA = [[NSMutableAttributedString alloc] initWithString:@"请输入想出售的总数量" attributes:attDicA];
    ...
    //---------
         ...
         NSMutableAttributedString *attPlace = [[NSMutableAttributedString alloc] initWithString:@"请输入想出售的总金额" attributes:attDic];
    ...
    //-------------
         ...
         [Qbutton setTitle:@"出售" forState:UIControlStateNormal];
         ...
         [Qbutton addTarget:self action:@selector(Add_chusou) forControlEvents:UIControlEventTouchUpInside];
...
//        [HeadPic addSubview:cc];
    ...
    //--------
         ...
         NSString * conter = [NSString stringWithFormat:@"说明备注： %@",_conter];
...
-(void)Add_chusou
{
    ...
    Add_order_s_ViewController *vc = [[Add_order_s_ViewController alloc]init];
...
-(void)json_pay
{
       ...
       NSString * username_s = [dic objectForKey:@"username"];
        ...
        NSString * Urladd =[NSString stringWithFormat:@"action=pay_list&username=%@",username_s];
        //NSLog(@"cc==%@",Urladd);
