...
@interface Add_chusou_ViewController ()<TopbarTimerPickerViewDelegate>

@end

@implementation Add_chusou_ViewController
...
//            //占位符颜色
       ...
       //--------
         ...
         if (indexPath.row == 0)
         {
            cell.textLabel.text = [NSString stringWithFormat:@"付款时间"];
            UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"app_back_btn_D"]];
            cell.accessoryView = imageView;
            Fk_time = [[UITextField alloc]initWithFrame:CGRectMake(130,10,200, 40)];
            Fk_time.textColor = [UIColor colorWithRed:5.0/255 green:33.1/255 blue:89.0/255 alpha:1];
            Fk_time.userInteractionEnabled = NO;// Pric_type.enabled = NO; 默认不可修改或辑编内容
            ...
            Fk_time.attributedPlaceholder = attPlaceA;
            [cell.contentView addSubview: Fk_time];
             ...
             {
            NSMutableDictionary *attDicA = [@{NSForegroundColorAttributeName:[UIColor colorWithRed:5.0/255 green:33.1/255 blue:89.0/255 alpha:0.2], NSFontAttributeName:[UIFont systemFontOfSize:16]} mutableCopy];
            NSMutableAttributedString *attPlaceA = [[NSMutableAttributedString alloc] initWithString:_Fk_time_A attributes:attDicA];
            Fk_time.attributedPlaceholder = attPlaceA;
            [cell.contentView addSubview: Fk_time];
             }
         }
...
-(void)Add_goumai
{
    ...
    NSString * Urladd =[NSString stringWithFormat:@"action=transaction&username=%@&type=0&dj_type=%@&Currency=%@&pri=%@&vol=%@&tatol=%@&reg_time=%@&fk_time=%@&rzdj=%@&conter=%@&Another=%@&LimitA=%@&alipay=%@&weixin=%@",username,@"固定价格",_titlea,Pric.text,Pric_vol.text,_Apri_total,_timeStr,_Fk_time_A,_KYC,Rz_con.text,Another,LimitA,alipay,weixin];  //tpe=1 是我要购买， type=0 是我要出售
...
-(void)Authentication
{
       // NSLog(@"btn_type===%@",_Btn_type);
        //
    if ([_Btn_type isEqualToString:@"0"]) {//同一按钮判断
        PopView  = [[Pop_rz alloc]init];
        PopView.type = @"1"; //传值，到底是认证时间还是认证等级，因为调用同一个窗口。 0 认证等级（默认）  、1 认证时间
        //三选其一显示
        ...
        //[PopView coutomshowInView:[UIApplication sharedApplication].keyWindow];
         ...
         __block Add_chusou_ViewController *blockSelf = self;//创建一个本地变量blockSelf，指向self，然后用结构体语法访问实例变量
...
//   HUD.square = YES;
    ...
    if ([_Btn_type isEqualToString:@"0"]) {
